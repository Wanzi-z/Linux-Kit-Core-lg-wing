 ...
 */
#include "touch_sub_sw42000.h"
#include "touch_sub_sw42000_prd.h"

#if defined(__SUPPORT_ABT)
#include "touch_sub_sw42000_abt.h"
#endif
...
static void project_param_set(struct device *dev)
{
	...
	param->touch_sub_power_control_en = FUNC_ON;
...
int sw42000_xfer_msg(struct device *dev, struct touch_sub_xfer_msg *xfer)
{
	struct sw42000_data *d = to_sw42000_data(dev);
	struct touch_sub_xfer_data_t *tx = NULL;
	struct touch_sub_xfer_data_t *rx = NULL;
	int addr_check = 0;
	int buf_cnt = 0;
	...
	ret = touch_sub_bus_xfer(dev, xfer);
	if (ret) {
...
int sw42000_reg_read(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	struct sw42000_data *d = to_sw42000_data(dev);
	struct touch_sub_bus_msg msg = {0, };
	int buf_cnt = 0;
	int r_hdr_size = R_HEADER_SIZE_I2C;
	int w_hdr_size = W_HEADER_SIZE_I2C;
	int ret = 0;
	...
	ret = touch_sub_bus_read(dev, &msg);
...
int sw42000_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	...
	ret = touch_sub_bus_write(dev, &msg);
	mutex_unlock(&d->io_lock);
...
int sw42000_hw_reset(struct device *dev, int mode)
{
	...
	touch_sub_gpio_direction_output(ts->reset_pin, 0);
	...
	if (mode == HW_RESET_ASYNC) {
		...
		ts->driver->init(dev);
		TOUCH_I("HW RESET SYNC!! - swivel_status : %d(boot_mode : %d)", atomic_read(&ts->state.swivel), boot_mode);
		if(boot_mode != TOUCH_NORMAL_BOOT || atomic_read(&ts->state.swivel) == SWIVEL_OPENED) {
...
int sw42000_only_hw_reset(struct device *dev)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
...
static int sw42000_power(struct device *dev, int ctrl)
{
...
#endif
		if (param->touch_sub_power_control_en == FUNC_ON) {
			TOUCH_I("%s, off\n", __func__);
			atomic_set(&d->init, IC_INIT_NEED);
			touch_sub_gpio_direction_output(ts->reset_pin, 0);
			touch_sub_power_1_8_vdd(dev, 0);
			touch_sub_power_3_3_vcl(dev, 0);
			touch_sub_msleep(1);
		} else {
	...
	case POWER_ON:
		if (param->touch_sub_power_control_en == FUNC_ON) {
			TOUCH_I("%s, on\n", __func__);
			touch_sub_power_3_3_vcl(dev, 1);
			touch_sub_power_1_8_vdd(dev, 1);
			touch_sub_msleep(10);
			touch_sub_gpio_direction_output(ts->reset_pin, 1);
		} else {
...
int sw42000_tc_driving(struct device *dev, int mode)
{
	...
	if ((mode != LCD_MODE_STOP) && (mode != d->driving_mode)) {
		...
		TOUCH_I("sw42000_tc_stop(0x%x) = LCD_MODE_STOP(%d), (0x%x)\n",
				addr, mode, ctrl);
...
static int sw42000_swivel_status(struct device *dev, u32 state)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	int boot_mode = 0;
	int prev_status = atomic_read(&ts->state.prev_swivel);
	int cur_status = atomic_read(&ts->state.swivel);
	TOUCH_TRACE();

	TOUCH_I("Swivel state: %d (Swivel prev_state : %d)\n", cur_status, prev_status);

	boot_mode = touch_sub_check_boot_mode(dev);
	if(boot_mode == TOUCH_NORMAL_BOOT)
	{
		if (cur_status == SWIVEL_HALF)
		{
			TOUCH_I("%s : HALF SWIVEL!!!! INTERRUPT DISABLE!!!",__func__);
			touch_sub_report_all_event(ts);
			touch_sub_interrupt_control(ts->dev, INTERRUPT_DISABLE);
		}
		else
		{
			if(atomic_read(&ts->state.sleep) != IC_DEEP_SLEEP)
			{
				TOUCH_I("%s : Swivel change complete!!!! INTERRUPT Enable",__func__);
				touch_sub_interrupt_control(ts->dev, INTERRUPT_ENABLE);
			}
			else
			{
				TOUCH_I("%s :  deep sleep status!!!! skip interrupt control!!",__func__);
			}
		}
	}
...
static void sw42000_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	...
	touch_sub_notifier_call_chain(NOTIFY_FB, &ret);
}
...
static int sw42000_notify(struct device *dev, ulong event, void *data)
{
	...
	case NOTIFY_FM_RADIO:
		...
		switch (d->driving_mode) {
			case LCD_MODE_U0:
				ctrl = 0x04;
				sw42000_sub_reg_write(dev, addr, &ctrl, sizeof(ctrl));
				touch_sub_msleep(param->sys_tc_stop_delay);
...
//					}
				...
				sw42000_sub_reg_write(dev, addr,
						&ctrl, sizeof(ctrl));
				TOUCH_I("sw42000 fm radio state write(0x%x) = (0x%x)\n",
						addr,  ctrl);
				/* tc_status check */
				sw42000_sub_reg_read(dev, TC_IC_STATUS, (u8 *)&rdata, sizeof(u32));
				TOUCH_I("read ic_status(%x) = %x\n", TC_IC_STATUS, rdata);
				break;

			case LCD_MODE_U3:
				TOUCH_I("Set FM Radio in U3:(%d)\n", *(u32 *)data);
				ret = sw42000_sub_reg_write(dev, ABT_CMD +
...
#endif
	case NOTIFY_SWIVEL:
		TOUCH_I("NOTIFY_SWIVEL!\n");
		ret = sw42000_swivel_status(dev, *(u32 *)data);
		break;
...
static int sw42000_probe(struct device *dev)
{
	...
	touch_sub_gpio_init(ts->int_pin, "touch_sub_int");
	touch_sub_gpio_direction_input(ts->int_pin);

	touch_sub_gpio_init(ts->maker_id_pin, "touch_sub_make_id");
	touch_sub_gpio_direction_input(ts->maker_id_pin);

	touch_sub_power_init(dev);
	touch_sub_bus_init(dev, MAX_BUF_SIZE);
...
static int sw42000_fw_compare(struct device *dev, const struct firmware *fw)
{
	...
	if ((d->ic_info.img_version.major == 0) && (d->ic_info.img_version.minor == 0) && (d->ic_info.img_version.build == 0)) {
		memcpy(&d->ic_info.img_version, &fw->data[bin_ver_offset], sizeof(struct sw42000_version_bin));
		TOUCH_I("%s : set initial built in f/w version\n", __func__);
	}
...
out:
	touch_sub_msleep(100);

	return ret;
}

static int sw42000_flash_erase_check(struct device *dev)
{
	...
	sw42000_reg_write(dev, spr_code_offset, &start_offset, sizeof(start_offset));
	TOUCH_I("flash read offset(start_offset) %x \n", start_offset);	//for debug

	sw42000_reg_read(dev, code_access_addr, &flash_read_val, sizeof(flash_read_val));
	...
	sw42000_reg_write(dev, spr_code_offset, &end_offset, sizeof(end_offset));
...
static int __used sw42000_fw_flash_mass_erase(struct device *dev)
{
	...
	sw42000_reg_read(dev, fc_ctrl, &read_reg, sizeof(read_reg));
	...
	sw42000_reg_read(dev, SYS_RST_CTL, &read_reg, sizeof(read_reg));
	TOUCH_I("1st cm3 status check read_val : %x\n", read_reg);

	//Flash Area check(first,last)
	ret = sw42000_flash_erase_check(dev);
	if (ret < 0) {
		fc_err = 5;
		TOUCH_E("sw42000_flash_erase_check(fc_err : %d) error!!!\n", fc_err);
		goto out;
	}
	...
	ret = sw42000_fw_flash_crc(dev, &erase_crc_val);
	if (ret < 0) {
		fc_err = 6;
		TOUCH_E("sw42000_flash_crc(fc_err : %d) error!!!\n", fc_err);
		goto out;
	}
...
static int sw42000_upgrade(struct device *dev)
{
	...
	d->check_fwup = sw42000_fw_compare(dev, fw);
	...
	if (d->check_fwup == UPGRADE_NEED || d->check_fwup == UPGRADE_FORCE) {
		...
		for (i = 0; i < 3 && ret; i++) {
			...
			if (ret < 0) {
				TOUCH_E("%s: sw42000_fw_upgrade failed!(ret = %d)\n", __func__, ret);
				sw42000_reset_ctrl(dev, HW_RESET_ONLY);
				//goto error;
			} else {
				TOUCH_I("%s: sw42000_fw_upgrade is done!\n", __func__);
			}
...
static int sw42000_app_upgrade(struct device *dev)
{
	struct touch_sub_core_data *ts = to_touch_core(dev);
	const struct firmware *fw = NULL;
...
static int sw42000_init(struct device *dev)
{
	...
	if (ret < 0) {
		...
		touch_sub_interrupt_control(dev, INTERRUPT_DISABLE);
		sw42000_power(dev, POWER_OFF);
		sw42000_power(dev, POWER_ON);
		touch_sub_msleep(ts->caps.hw_reset_delay);
		return ret;
		...
		size_t count)
{
	...
	touch_sub_interrupt_control(dev, INTERRUPT_ENABLE);
...
//	&touch_sub_attr_ai_pick.attr,
	&touch_sub_attr_ts_noise_log_enable.attr,
...
static int sw42000_sub_register_sysfs(struct device *dev)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	int ret = 0;
	...
	TOUCH_I("sw42000 sysfs reigster\n");
	...
	ret = sw42000_sub_prd_register_sysfs(dev);
#if defined(__SUPPORT_ABT)
	ret = sw42000_sub_abt_register_sysfs(dev);
#endif
	if (ret < 0) {
...
static struct touch_sub_driver touch_sub_driver = {
	.probe = sw42000_probe,
...
#if defined(CONFIG_LGE_TOUCH_SUB_APP_FW_UPGRADE)
	.app_upgrade = sw42000_app_upgrade,
#endif
...
//	.swipe_enable = sw42000_swipe_enable,
	...
	.register_sysfs = sw42000_sub_register_sysfs,
	.set = sw42000_set,
	.get = sw42000_get,
};

#define MATCH_NAME_SUB		"lge_sub,sw42000_sub"

static struct of_device_id touch_sub_match_ids[] = {
	{ .compatible = MATCH_NAME_SUB, },
	{},
};

static struct touch_sub_hwif hwif = {
	.bus_type = HWIF_I2C,
	.name = LGE_TOUCH_SUB_NAME,
	.owner = THIS_MODULE,
	.of_match_table = of_match_ptr(touch_sub_match_ids),
};

extern bool panel_type_touch(char *panel_name);

static int __init touch_sub_device_init(void)
{
	TOUCH_TRACE();
/*
...
*/
	...
	// if (!is_ddic_name(1,"rm692C9")) {
	// 	TOUCH_I("%s, ddic rm692C9 not found.\n", __func__);
	// 	return 0;
	// }
...
#endif
	TOUCH_I("%s, sw42000 detected.\n", __func__);


	return touch_sub_bus_device_init(&hwif, &touch_sub_driver);
}

static void __exit touch_sub_device_exit(void)
{
	TOUCH_TRACE();
	touch_sub_bus_device_exit(&hwif);

	TOUCH_I("%s, sw42000 ends\n", __func__);
}

late_initcall(touch_sub_device_init);
module_exit(touch_sub_device_exit);
