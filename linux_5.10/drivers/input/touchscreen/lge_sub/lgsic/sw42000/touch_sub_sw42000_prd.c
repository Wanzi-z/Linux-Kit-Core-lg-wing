...
static void log_file_size_check(struct device *dev)
{
	...
	case TOUCH_NORMAL_BOOT:
		fname = "/data/vendor/touch/touch_sub_self_test.txt";
		break;
	case TOUCH_MINIOS_AAT:
		fname = "/data/touch/touch_sub_self_test.txt";
		break;
	case TOUCH_MINIOS_MFTS_FOLDER:
	case TOUCH_MINIOS_MFTS_FLAT:
	case TOUCH_MINIOS_MFTS_CURVED:
		fname = "/data/touch/touch_sub_self_mfts.txt";
		break;
	default:
...
static int prd_short_result_get(struct device *dev, int frame)
{
	...
	case PT_FRAME_1:
		...
		ret = prd_read_print_frame(dev, param->offset.frame_1_offset,
			ROW_SIZE, COL_SIZE, VERTICAL_DATA);
        */
        ret = prd_read_self_print(dev, param->u3_m2_self, ROW_SIZE, COL_SIZE, 0);
		break;
	case PT_FRAME_2:
		TOUCH_E("Attempt to read invalid frame\n");
		break;
...
static int tune_u8_print(int num_ch, u8 *tune_val, char *log_buf, int offset)
{
	int i;
	int temp = 0;
...
{
	struct touch_sub_core_data *ts = container_of(kobj, struct touch_sub_core_data, kobj);
...
#define TOUCH_BIN_ATTR(_name, _read, _write, _size)		\
			struct bin_attribute touch_sub_bin_attr_##_name	\
...
static struct attribute *prd_attribute_list[] = {
	&touch_sub_attr_sd.attr,
	&touch_sub_attr_fdata.attr,
	&touch_sub_attr_lpwg_sd.attr,
	&touch_sub_attr_psy_test.attr,

	/* [Start] for siw app */
	&touch_sub_attr_prd_app_raw.attr,
	&touch_sub_attr_prd_app_base.attr,
	&touch_sub_attr_prd_app_label.attr,
	&touch_sub_attr_prd_app_delta.attr,
	&touch_sub_attr_prd_app_debug_buf.attr,
	&touch_sub_attr_prd_app_end.attr,
	&touch_sub_attr_prd_app_info.attr,
	/* [End] for siw app */
	NULL,
};
...
static struct bin_attribute *prd_attribute_bin_list[] = {
	//mutual
	&touch_sub_bin_attr_delta,
	&touch_sub_bin_attr_rawdata,
	&touch_sub_bin_attr_base,
	&touch_sub_bin_attr_debug,
	&touch_sub_bin_attr_label,
	//mutual+self
	&touch_sub_bin_attr_delta_s,
	&touch_sub_bin_attr_rawdata_s,
	&touch_sub_bin_attr_base_s,
	&touch_sub_bin_attr_debug_s,
	&touch_sub_bin_attr_label_s,
	NULL,
};
...
int sw42000_sub_prd_register_sysfs(struct device *dev)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	struct sw42000_data *d = to_sw42000_data(dev);
	struct siw_hal_prd_data *prd = (struct siw_hal_prd_data *)d->prd;
